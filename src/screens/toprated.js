...
class Toprated extends Component {
    ...
    fetchPopularMovie = async (currentPage) => {
        const popularMovies = await tmdb.get("/top_rated", {
            params: {
                language: 'en-US',
                page: currentPage
            }
                    ...
                    <div className={"movie__list"}>
                        {
                            this.state.results.map((movie, index) => {
                                ...
                                return <MovieCard title={`${movie.original_title}`} imageUrl={`https://image.tmdb.org/t/p/w185${movie.poster_path}`} updateVideoId={this.updateVideoId} movieId={movie.id} key={movie.id} />
